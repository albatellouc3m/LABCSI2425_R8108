...
def login():
    if request.method == "POST":
        ...
        # Supongamos que tienes una función en `lol.py` que autentifica al usuario
        from lol import autentificar_usuario
...
def register_user():
    ...
    # Llamar a la función registrar_usuario de lol.py
...
def authenticate_user():
    ...
    # Llamar a la función autentificar_usuario de lol.py
    status, message = lol.autentificar_usuario(username, password)
    if status == 0:
...
def mostrar_test(name_test):
    ...
    preguntas = lol.obtener_preguntas(name_test)
...
def guardar_respuestas():
    ...
    # Llamar a la función en lol.py que guarda las respuestas y calcula el resultado
    status, message, result = lol.guardar_y_calcular_resultado(username, name_test, preguntas, respuestas)

    if status == 0:
        flash(message, "success")
        return render_template("mostrar_resultado.html", name_test=name_test, result=result)
    else:
        flash(message, "danger")
        return redirect("/home")
...
def ver_respuestas(name_test):
    ...
    respuestas = lol.obtener_respuestas(name_test, username)
