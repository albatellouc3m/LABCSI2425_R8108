...
def guardar_y_calcular_resultado(username, name_test, preguntas, respuestas):
    ...
    if status == 0:  # Si las respuestas se guardaron correctamente
        try:
            ...
            sql.cursor.execute("SELECT result FROM Results WHERE username = %s AND name_test = %s ORDER BY date_result DESC LIMIT 1", (username, name_test))
            resultado = sql.cursor.fetchone()[0]  # Tomar el resultado más reciente
            ...
            return (0, "Respuestas guardadas y resultado calculado correctamente", resultado_encriptado)
        except Exception as e:
            sql.db.rollback()  # Revertir en caso de error durante el cálculo del resultado
            return (1, f"Error al calcular el resultado: {e}", None)
    else:
        return (1, message, None)  # Retornar el error si no se pudieron guardar las respuestas
